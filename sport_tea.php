...
class sport_res{
	...
	function all(){
		...
		$SQL="select * from `sport_res` where idclass like '{$this->nowClass}%' and mid='{$this->Mid}' and kmaster!='2' order by idclass ,itemid ";//and kgp='0'
		$rs=$this->CONN->query($SQL) or die($SQL);
		$arr=$rs->fetchAll();
		$this->okStu=$arr;
		if (count($arr)!=0){
	...
	function add(){
		...
		if ($tmpItem=='all'){
			...
			$rs=$this->CONN->query($in_sql) or die($in_sql);
			Header("Location:$URL");
			exit;
		}
		//拆開-- 項目 ,類別
		...
		foreach ($_POST['stu'] as $K=>$str){
			...
			//男分不分組
			if (substr($Im['enterclass'],-1)!='c'){
				if ($Im['enterclass']!=$CK) backe('報名組別錯誤！');
			}
			//$mid=1;
