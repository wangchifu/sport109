...
*/
	...
	function add(){
		...
		if (@count($_POST['spitem'])==0) backe('請選擇輸出組別！');
		$tmp=array();
		foreach ($_POST['spitem'] as $K=>$V){
			if ($K!=$V) backe('資料錯誤！');
			$tmp[]=$K;
		}
		$IM=join(',',$tmp);
		$Order_syntax='order by itemid ,sportorder ';
		$SQL="select * from `sport_res`  where mid='{$this->Mid}' and itemid in ($IM)  $Order_syntax  ";
		$rs=$this->CONN->query($SQL) or die($SQL);
		$arr=$rs->fetchAll(PDO::FETCH_ASSOC);
		$tmp=array();
		foreach ($arr as $ar){
		...
		$tpl = __SitePtah."/sport_view_P.htm";
		$this->smarty->assign("this",$this);
		$this->smarty->display($tpl);
		//PP($tmp);
...
exit;
	...
	function myBox5($stu,$IM){
		...
		foreach ($stu as $ar){
			if ($ar['kmaster']=='2') {
				//$head[]=$ar;
				echo $ar['cname'].':(道次'.($ar['sportorder']+$offset).')&nbsp;';
				$cla=$ar['idclass'];
				$GG=$ar['kgp'];
				foreach ($stu as $ar2){
				//if ($ar2['kmaster']!='2' && substr($ar2['idclass'],0,3)==$cla) {
				if ($ar2['kmaster']=='0' && $ar2['kgp']==$GG && substr($ar2['idclass'],0,3)==$cla) {
					echo $ar2['sportorder'].'棒:'.$ar2['sportnum'].$ar2['cname'].'&nbsp;';
					}
