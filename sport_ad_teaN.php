...
class sport_res{
	...
	/* 4.顯示畫面處理函式*/
	function display(){
		$tpl = __SitePtah."/sport_ad_teaN.htm";
	...
	//PP($_POST);	//$fields=array('id','mid','itemid','kmaster','kgp','kend','stud_id','sportkind','cname','idclass','sportnum','num','results','sportorder','memo','passOK');
...
//$fields=array('id','mid','itemid','kmaster','kgp','kend','stud_id','sportkind','cname','idclass','sportnum','num','results','sportorder','memo','passOK');
	...
	function myBox1a($name,$stu,$N){
		...
		$str.=$Val."<input class=bt2 type='text' size=1 name='".$name."[".$K."]' value='".$ar['sportnum']."'>\n";
		if ($i%$N==0)$str.='<br>';
		$i++;
		}
		return $str;
